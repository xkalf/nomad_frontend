import { type Packet, PacketInvalid, PacketStatus } from '../packet/packet'
import { isEvent, type TimerEvent } from './timer-event'
  ...
  public receivePacket(packet: Packet): void {
    ...
    if (this.lastPacket) {
      ...
      if (
        ...
        (packet.status === PacketStatus.RUNNING ||
          (this.lastPacket.timeInMilliseconds === 0 && packet.timeInMilliseconds > 0))
      ) {
