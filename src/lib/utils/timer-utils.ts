import { SolveStatus, type Solve } from '$lib/types'
...
export const getBest = (arr: Solve[]) => {
	...
	const best = Math.min(...arr.filter(i => i.status !== SolveStatus.Dnf).map(i => i.time))

	return displayTime(best)
}

export const getAvg = (arr: Solve[]) => {
	if (arr.filter(i => i.status === SolveStatus.Dnf).length > 2) {
		return 'DNF'
	}

	const sum = arr.filter(i => i.status !== SolveStatus.Dnf).reduce((a, b) => (a += b.time), 0)
	const max = arr.find(i => i.status === SolveStatus.Dnf) ? 0 : Math.max(...arr.map(i => i.time))
