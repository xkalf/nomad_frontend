...
{#if solves && solves.length === count}
	<dialog bind:this={modal} class="rounded-xl bg-background py-4 px-4 font-sans text-primary md:px-0">
		<div class="mx-auto w-full md:w-3/5">
			<h2 class="text-center text-2xl md:text-3xl">Avg status</h2>
			<p class="text-center">Generated By Nomad timer on {today}</p>
			<div class="mx-auto mt-6">
				<div class="flex w-full items-center rounded-xl bg-secondary text-center text-lg text-white">
					<div
						class={`rounded-xl bg-primary p-4 md:w-1/5 ${label.length > 5 ? 'w-2/5' : 'w-1/5'}`}
					>
						{label}
					</div>
			...
			<form method="dialog" class="mt-2 flex justify-center gap-4">
				<button class="rounded-xl bg-primary py-2 px-4 text-xl text-white" value="close">Close</button>
				<button class="rounded-xl bg-primary py-2 px-4 text-xl text-white" on:click={copyClipboard}>Copy</button>
			</form>
		</div>
	</dialog>
{/if}
