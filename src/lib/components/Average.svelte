	...
	async function copyClipboard() {
		if (solves && count) {
			...
			await navigator.clipboard.writeText(
				`Generated By NomadTimer on ${date.getFullYear()}-${pad(date.getMonth())}-${pad(
					date.getDate()
				)}\navg of ${count}: ${getAvg(solves, count)}\n\nTime List:\n${solves
	...
	<dialog bind:this={modal} class="rounded-xl bg-[#454F57] py-4 px-4 font-sans text-white md:px-0">
		<div class="mx-auto w-full md:w-3/5">
			...
			<div class="mx-auto mt-6">
				<div class="relative w-full rounded-xl bg-[#272A2D] p-4 text-center text-lg">
					<div
						class={`absolute left-0 top-0 text-ellipsis whitespace-nowrap rounded-xl bg-[#33393E] p-4 md:w-1/5 ${
			...
			<h3 class="mt-8 text-xl">Time list</h3>
