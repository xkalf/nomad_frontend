...
{#if solves && solves.length === count}
	<dialog
		bind:this={modal}
		class="rounded-xl bg-background py-4 px-4 font-sans text-primary md:px-0"
	>
		<div class="mx-auto w-full md:w-3/5">
			...
			<div class="mt-2 max-h-72 overflow-y-auto pr-4 text-2xl">
			...
			<form method="dialog" class="mt-2 flex justify-center gap-4">
				<button class="rounded-xl bg-primary py-2 px-4 text-xl text-white" value="close"
					>Close</button
				>
